...
class Rider with _$Rider {
  ...
  const factory Rider({
    ...
    @Default(ProfileVerificationStatus.unverified) ProfileVerificationStatus verificationStatus,
  ...
  Rider merge(Rider? value) => copyWith(
        ...
        firstName: value?.firstName.isNotNull((it) => it as DisplayName, orElse: (_) => firstName) ?? firstName,
        lastName: value?.lastName.isNotNull((it) => it as DisplayName, orElse: (_) => lastName) ?? lastName,
        email: value?.email.isNotNull((it) => it as EmailAddress, orElse: (_) => email) ?? email,
        password: value?.password.isNotNull((it) => it as Password, orElse: (_) => password) ?? password,
        phone: value?.phone.isNotNull((it) => it as Phone, orElse: (_) => phone) ?? phone,
        photo: value?.photo.isNotNull((it) => it as MediaField, orElse: (_) => photo) ?? photo,
        location: value?.location ?? location,
        availability: value?.availability ?? availability,
        flexible: value?.flexible ?? flexible,
        verificationStatus: value?.verificationStatus ?? verificationStatus,
        phoneVerified: value?.phoneVerified ?? phoneVerified,
        avgRating: value?.avgRating.isNotNull((it) => it as BasicTextField<double?>, orElse: (_) => avgRating) ?? avgRating,
